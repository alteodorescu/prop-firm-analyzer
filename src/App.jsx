    ...
    <div className={`bg-white rounded-xl shadow-sm border-2 ${easeBorder(f.overallEase)} overflow-hidden transition-all`}>
      <div className="p-4">
        ...
        <div className="flex justify-end gap-2 mt-3">
          <button onClick={() => onEdit(f)} className="flex items-center gap-1 text-xs text-blue-600 hover:text-blue-800 px-2 py-1 rounded hover:bg-blue-50 transition-colors">
            <Pencil size={12} /> {t("edit")}
          </button>
          <button onClick={() => onDelete(f.id)} className="flex items-center gap-1 text-xs text-red-500 hover:text-red-700 px-2 py-1 rounded hover:bg-red-50 transition-colors">
      ...
      <div className="bg-slate-800 text-white">
        <div className="max-w-6xl mx-auto px-4 py-4">
          ...
          <div className="flex gap-1 mt-3 -mb-px">
            ...
            <button onClick={() => setTab("tracker")} className={`px-4 py-2 text-sm font-medium rounded-t-lg transition-colors ${tab === "tracker" ? "bg-gray-50 text-slate-800" : "text-slate-400 hover:text-white hover:bg-slate-700"}`}>
              {t("tabTracker")}{accounts.length > 0 ? ` (${accounts.filter(a => a.status !== "archived").length})` : ""}
            </button>
            <button onClick={() => setTab("dashboard")} className={`px-4 py-2 text-sm font-medium rounded-t-lg transition-colors ${tab === "dashboard" ? "bg-gray-50 text-slate-800" : "text-slate-400 hover:text-white hover:bg-slate-700"}`}>
              {t("tabDashboard")}
            </button>
          ...
          <>
            ...
            <div className="space-y-3">
              {computed.map((firm, i) => (
                <div key={firm.id} id={`firm-${firm.id}`}>
                  <FirmCard firm={firm} rank={i + 1} onEdit={f => setEditing(f)} onDelete={handleDelete} />
                </div>
              ))}
            </div>
            ...
            {computed.length === 0 && (
              <div className="text-center py-16 text-gray-400">
                ...
                <p className="text-sm mt-1">{t("clickAddFirm")}</p>
              </div>
            )}
          </>
        )}
